...
### BUG-15: ~~`<ui-tree>` is fundamentally broken~~ FIXED

**Root cause:** The tree used DOM nesting for hierarchy and CSS `paddingLeft` for indentation. Collapsed state only filtered `getVisibleItems()` but didn't hide DOM children. The indicator `<span>` was prepended each frame, eating into text width.

**Fix:** Rewrote the tree to flatten its rendering. Source `<ui-tree-item>` elements are captured on connect and removed from the DOM. The tree computes the visible flat list from the logical hierarchy, then renders simple `<div>` rows with text-based indentation (spaces + indicator + label). Expand/collapse rebuilds the flat list and re-renders. 12 unit tests.
