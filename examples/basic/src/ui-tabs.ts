import {Tabs, Tab} from '@cliui/elements';
...
const tabs1 = document.createElement('tabs') as InstanceType<typeof Tabs>;
...
tabs1.addEventListener('input', () => {
  status.textContent = `Active: ${names1[(tabs1 as Tabs).getActiveIndex()]}`;
});
...
const tabs2 = document.createElement('tabs') as InstanceType<typeof Tabs>;
...
tabs2.addEventListener('input', () => {
  status.textContent = `Many tabs: active index ${(tabs2 as Tabs).getActiveIndex()}`;
});
...
const tabs3 = document.createElement('tabs') as InstanceType<typeof Tabs>;
tabs3.setAttribute('tabindex', '0');
const singleTab = document.createElement('tab');
