...
describe('UiSelect mouse interactions via EventDispatcher', () => {
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it('clicking an option does not blur the select', async () => {
    ...
    const optA = terminal.document.createElement('ui-option') as UiOption;
    optA.setAttribute('value', 'a');
    optA.textContent = 'Alpha';

    const optB = terminal.document.createElement('ui-option') as UiOption;
