import {describe, expect, it} from 'vitest';

import {Window} from '../../classes/Window';
import {CommentNodeGuard} from '../CommentNodeGuard';

describe('CommentNodeGuard', () => {
  it('returns true for a comment node', () => {
    const window = new Window();
    const comment = window.document.createComment('content');

    expect(CommentNodeGuard(comment)).toBe(true);
  });

  it('returns false for a text node', () => {
    const window = new Window();
    const text = window.document.createTextNode('content');

    expect(CommentNodeGuard(text)).toBe(false);
  });

  it('returns false for an element node', () => {
    const window = new Window();
    const element = window.document.createElement('div');

    expect(CommentNodeGuard(element)).toBe(false);
  });

  it('returns false for a document fragment', () => {
    const window = new Window();
    const fragment = window.document.createDocumentFragment();

    expect(CommentNodeGuard(fragment)).toBe(false);
  });
});
