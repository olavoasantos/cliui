...
import {Tbody} from '../component';

function createSection(rows: string[][] = []): {window: Window; section: Tbody} {
  ...
  const section = window.document.createElement('tbody') as Tbody;
...
describe('Tbody', () => {
  it('registers the custom element under its tag name', () => {
    ...
    expect(window.customElements.get('tbody')).toBe(Tbody as unknown as CustomElementConstructor);
  });
