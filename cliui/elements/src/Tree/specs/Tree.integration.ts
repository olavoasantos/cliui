...
describe('Tree integration', () => {
  ...
  it('renders tree items with full text visible', async () => {
    ...
    terminal.window.customElements.define(Tree.tagName, Tree);
    terminal.window.customElements.define(TreeItem.tagName, TreeItem);

    const doc = terminal.document;
    const tree = doc.createElement('tree') as Tree;
    tree.setAttribute('tabindex', '0');

    const item1 = doc.createElement('treeitem');
    item1.setAttribute('value', 'Alpha');
    item1.textContent = 'Alpha';

    const item2 = doc.createElement('treeitem');
