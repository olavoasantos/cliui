...
function createDetails(
  ...
  window.customElements.define(Details.tagName, Details as unknown as CustomElementConstructor);

  const details = window.document.createElement('details') as Details;
...
function keyDown(details: Details, key: string): KeyboardEvent {
...
describe('Details', () => {
  it('registers the custom element under its tag name', () => {
    ...
    expect(window.customElements.get('details')).toBe(
      Details as unknown as CustomElementConstructor,
    );
  });
