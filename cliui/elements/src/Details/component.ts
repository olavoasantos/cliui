...
import {
  DETAILS_INDICATOR_COLLAPSED,
  DETAILS_INDICATOR_EXPANDED,
  DETAILS_OBSERVED_ATTRIBUTES,
  DETAILS_TAG_NAME,
} from './constants';
import {Event, HTMLElement, ToggleEvent, type Element, type KeyboardEvent, type Node} from '@cliui/dom';
 ...
 * - The first child with `localName === 'summary'` acts as the
 ...
 *
 * Register with `window.customElements.define(Details.tagName, Details)`
 * before creating `<details>` elements in a window.
 */
export class Details extends HTMLElement {
  static override readonly observedAttributes = DETAILS_OBSERVED_ATTRIBUTES;
  static readonly styles = styles;
  static readonly tagName = DETAILS_TAG_NAME;
  ...
  private buildInternals(): void {
    ...
    this.summaryRow.setAttribute('class', 'details-summary');
    ...
    this.contentWrapper.setAttribute('class', 'details-content');
  ...
  private findSummaryChild(): Element | null {
    for (let i = 0; i < this.childNodes.length; i++) {
      ...
      if (child && 'localName' in child && (child as Element).localName === 'summary') {
