...
import {TABS_OBSERVED_ATTRIBUTES, TABS_TAG_NAME} from './constants';
 ...
 *
 * Contains `<tab title="...">content</tab>` children. Renders a
 ...
 *   <tab title="Details">Details content</tab>
 * </tabs>
 * ```
 *
 * Register with `window.customElements.define(Tabs.tagName, Tabs)`
 * before creating `<tabs>` elements in a window.
 */
export class Tabs extends HTMLElement {
  static override readonly observedAttributes = TABS_OBSERVED_ATTRIBUTES;
  ...
  private buildTabBar(): void {
    ...
    this.tabBar.setAttribute('class', 'tabs-bar');
  ...
  private getTabs(): Tab[] {
    ...
    for (let i = 0; i < this.childNodes.length; i++) {
      ...
      if (child && 'localName' in child && (child as any).localName === 'tab') {
        tabs.push(child as unknown as Tab);
      }
    }
