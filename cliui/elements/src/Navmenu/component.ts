 ...
 *
 * A positioned list of `<navmenuitem>` elements with keyboard navigation.
 ...
 *
 * Register with `window.customElements.define(Navmenu.tagName, Navmenu)`
 * before creating `<navmenu>` elements in a window.
 */
export class Navmenu extends HTMLElement {
  static override readonly observedAttributes = NAVMENU_OBSERVED_ATTRIBUTES;
  static readonly styles = styles;
  static readonly tagName = NAVMENU_TAG_NAME;
  ...
  close(): void {
    ...
    if (parent && parent.localName === 'dropdown') {
      const doc = this.ownerDocument as Document;
  ...
  private getItems(): NavmenuItem[] {
    const items: NavmenuItem[] = [];

    for (let i = 0; i < this.children.length; i++) {
      const child = this.children[i] as Element;

      if (child.localName === 'navmenuitem' && !child.hasAttribute('disabled')) {
        items.push(child as unknown as NavmenuItem);
      }
    }
  ...
  private handleClick(event: Event): void {
    ...
    while (current && current !== (this as unknown as Element)) {
      if (current.localName === 'navmenuitem' && !current.hasAttribute('disabled')) {
        // Find the index among non-disabled items
        const items = this.getItems();
        const idx = items.indexOf(current as unknown as NavmenuItem);
