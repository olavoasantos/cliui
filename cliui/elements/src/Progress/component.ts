...
import {
  ...
  DEFAULT_PROGRESS_VARIANT_NAME,
  DEFAULT_PROGRESS_WIDTH,
  MIN_PROGRESS_WIDTH,
  PROGRESS_EQUILIBRIUM_DISTANCE,
  PROGRESS_EQUILIBRIUM_VELOCITY,
  PROGRESS_OBSERVED_ATTRIBUTES,
  PROGRESS_PART_SEPARATOR,
  PROGRESS_SPRING_DAMPING,
  PROGRESS_SPRING_EPSILON,
  PROGRESS_SPRING_FREQUENCY,
  PROGRESS_STATIC_SPRING,
  PROGRESS_TAG_NAME,
  PROGRESS_VARIANTS,
} from './constants';
...
import type {ProgressSpringCoefficients, ProgressVariantName} from './types';

/**
 * Built-in animated terminal progress bar custom element.
 *
 * Register with `window.customElements.define(Progress.tagName, Progress)`
 * before creating `<progress>` elements in a window.
 */
export class Progress extends HTMLElement implements TerminalFrameAware {
  static override readonly observedAttributes = PROGRESS_OBSERVED_ATTRIBUTES;
  static readonly styles = styles;
  static readonly tagName = PROGRESS_TAG_NAME;
  ...
  private createSpringCoefficients(deltaTime: number): ProgressSpringCoefficients {
    const angularFrequency = Math.max(0, PROGRESS_SPRING_FREQUENCY);
    const dampingRatio = Math.max(0, PROGRESS_SPRING_DAMPING);

    if (angularFrequency < PROGRESS_SPRING_EPSILON) {
      return PROGRESS_STATIC_SPRING;
    }

    if (dampingRatio > 1 + PROGRESS_SPRING_EPSILON) {
  ...
  private getEmptyChar(): string {
    return this.getAttribute('empty-char') ?? PROGRESS_VARIANTS[this.getVariantName()].emptyChar;
  }

  private getFillChar(): string {
    return this.getAttribute('fill-char') ?? PROGRESS_VARIANTS[this.getVariantName()].fillChar;
  }
  ...
  private getMax(): number {
    ...
    const parsedMax = rawMax == null ? DEFAULT_PROGRESS_MAX : Number(rawMax);
  ...
  private getVariantName(): ProgressVariantName {
    const rawVariant = this.getAttribute('variant');

    if (rawVariant == null || !(rawVariant in PROGRESS_VARIANTS)) {
      return DEFAULT_PROGRESS_VARIANT_NAME;
    }

    return rawVariant as ProgressVariantName;
  }
  ...
  private getWidth(): number {
    ...
    if (!Number.isFinite(parsedWidth) || parsedWidth < MIN_PROGRESS_WIDTH) {
      return DEFAULT_PROGRESS_WIDTH;
    }
  ...
  private isAnimating(): boolean {
    ...
    return !(
      distance < PROGRESS_EQUILIBRIUM_DISTANCE &&
      Math.abs(this.velocity) < PROGRESS_EQUILIBRIUM_VELOCITY
    );
  }

  private renderProgress(): void {
    this.textContent = this.getDisplayParts().join(PROGRESS_PART_SEPARATOR);
  }
