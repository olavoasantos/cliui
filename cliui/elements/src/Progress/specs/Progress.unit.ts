...
function createProgress(window = new Window(), attributes: Record<string, string | boolean> = {}) {
  window.customElements.define(Progress.tagName, Progress as unknown as CustomElementConstructor);
  const progress = window.document.createElement('progress') as Progress;
...
describe('Progress', () => {
  it('registers the custom element under its tag name', () => {
    ...
    expect(window.customElements.get('progress')).toBe(
      Progress as unknown as CustomElementConstructor,
    );
  });
