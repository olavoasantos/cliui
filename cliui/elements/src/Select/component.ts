...
import {
  ...
  SELECT_INDICATOR_DOWN,
  SELECT_INDICATOR_UP,
  SELECT_LISTBOX_Z_INDEX,
 ...
 *
 * Contains `<option>` children. When collapsed, shows the selected
 ...
 *
 * Register with `window.customElements.define(Select.tagName, Select)`
 * before creating `<select>` elements in a window.
 */
export class Select extends HTMLElement {
  static override readonly observedAttributes = SELECT_OBSERVED_ATTRIBUTES;
  static readonly styles = styles;
  static readonly tagName = SELECT_TAG_NAME;
  ...
  private buildInternals(): void {
    ...
    this.trigger.setAttribute('class', 'select-trigger');
    ...
    this.listbox.setAttribute('class', 'select-listbox');
    this.listbox.style.position = 'absolute';
    this.listbox.style.top = '1';
    this.listbox.style.zIndex = String(SELECT_LISTBOX_Z_INDEX);
    ...
    for (const child of topLevelChildren) {
      if (child.localName === 'optgroup') {
        ...
        header.setAttribute('class', 'optgroup-label');
        header.style.display = 'block';
        header.style.fontWeight = 'bold';
        header.style.paddingLeft = '1';
        header.style.zIndex = String(SELECT_LISTBOX_Z_INDEX);
        ...
        for (let i = 0; i < child.childNodes.length; i++) {
          ...
          if (
            ...
            (grandchild as Element).localName === 'option'
          ) {
        ...
        for (const option of groupOptions) {
          option.style.zIndex = String(SELECT_LISTBOX_Z_INDEX);
        ...
        child.style.zIndex = String(SELECT_LISTBOX_Z_INDEX);
  ...
  private getOptions(): Option[] {
    ...
    for (let i = 0; i < source.childNodes.length; i++) {
      ...
      if (child && 'localName' in child && (child as Element).localName === 'option') {
        options.push(child as unknown as Option);
      }
    }
  ...
  /**
   * Collects direct children that are either option or optgroup
  ...
  private collectTopLevelChildren(): Element[] {
    ...
    for (let i = 0; i < this.childNodes.length; i++) {
      ...
      if (
        child &&
        'localName' in child &&
        ((child as Element).localName === 'option' || (child as Element).localName === 'optgroup')
      ) {
  ...
  private findOptionFromTarget(target: Element): Option | null {
    let current: Element | null = target;

    while (current && current !== (this as unknown as Element)) {
      if (current.localName === 'option') {
        return current as unknown as Option;
      }
