...
function createSelect(
...
): {window: Window; select: Select; options: Option[]} {
  ...
  const select = window.document.createElement('select') as Select;
  ...
  for (const data of optionData) {
    const option = window.document.createElement('option') as Option;
...
function keyDown(element: Select, key: string): KeyboardEvent {
...
describe('Select', () => {
  it('registers the custom element under its tag name', () => {
    ...
    expect(window.customElements.get('select')).toBe(Select as unknown as CustomElementConstructor);
  });
