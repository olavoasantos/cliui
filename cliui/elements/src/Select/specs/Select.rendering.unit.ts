...
import {CellBuffer} from '@cliui/terminal';
...
function setup(opts: {value?: string; options: Array<{value: string; label: string}>}) {
  ...
  w.customElements.define(Select.tagName, Select as unknown as CustomElementConstructor);
  w.customElements.define(Option.tagName, Option as unknown as CustomElementConstructor);
  ...
  const select = d.createElement('select') as Select;
  if (opts.value) select.setAttribute('value', opts.value);

  for (const data of opts.options) {
    const opt = d.createElement('option') as Option;
...
describe('Select rendering (real example CSS)', () => {
...
describe('Select click behavior', () => {
  it('clicking an option selects it and closes dropdown', () => {
    ...
    const options = Array.from(
      ...
      if ((child as Element).localName === 'div') {
        ...
        ).filter((n) => (n as Element).localName === 'option') as unknown as Option[];
      }
